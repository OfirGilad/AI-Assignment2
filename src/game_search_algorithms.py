...
class GameAlgorithms:
    ...
    def alpha_beta_decision(self, game_node: GameNode) -> str:
        ...
        for child in game_node.children:
            ...
            min_value = self.min_value(
                ...
                alpha=alpha,
                beta=beta,
                cutoff_threshold=self.cutoff_threshold-1
            )
    ...
    def max_value(self, game_node: GameNode, alpha, beta, cutoff_threshold: int) -> float:
        if game_node.state.is_goal_state():
            max_value = game_node.state.game_mode_score(agent_idx=self.agent_idx, cutoff=False)
            return max_value

        if cutoff_threshold <= 0:
            max_value = game_node.state.game_mode_score(agent_idx=self.agent_idx, cutoff=True)
            return max_value
    ...
    def min_value(self, game_node: GameNode, alpha, beta, cutoff_threshold: int) -> float:
        if game_node.state.is_goal_state():
            min_value = game_node.state.game_mode_score(agent_idx=self.agent_idx, cutoff=False)
            return min_value

        if cutoff_threshold <= 0:
            min_value = game_node.state.game_mode_score(agent_idx=self.agent_idx, cutoff=True)
            return min_value
    ...
    def max_value_semi_cooperative(self, game_node: GameNode, cutoff_threshold: int) -> dict:
        if game_node.state.is_goal_state():
            max_dict = game_node.state.game_mode_score(agent_idx=self.agent_idx, cutoff=False)
            return max_dict

        if cutoff_threshold <= 0:
            max_dict = game_node.state.game_mode_score(agent_idx=self.agent_idx, cutoff=True)
            return max_dict
    ...
    def min_value_semi_cooperative(self, game_node: GameNode, cutoff_threshold: int) -> dict:
        if game_node.state.is_goal_state():
            min_dict = game_node.state.game_mode_score(agent_idx=self.agent_idx, cutoff=False)
            return min_dict

        if cutoff_threshold <= 0:
            min_dict = game_node.state.game_mode_score(agent_idx=self.agent_idx, cutoff=True)
            return min_dict
    ...
    # Fully Cooperative #
    #####################
    def fully_cooperative_decision(self, game_node: GameNode) -> float:
    ...
    def maximax_value(self, game_node: GameNode, cutoff_threshold: int) -> float:
        if game_node.state.is_goal_state():
            max_max_value = game_node.state.game_mode_score(agent_idx=game_node.state.agent_idx, cutoff=False)
            return max_max_value

        if cutoff_threshold <= 0:
            max_max_value = game_node.state.game_mode_score(agent_idx=game_node.state.agent_idx, cutoff=True)
            return max_max_value
